...
bool		ABoneObj::Frame()
{
	m_fElapseTime += (g_fSPF * m_fSpeed *  m_Scene.iFrameSpeed);
...
bool		ABoneObj::Render(ID3D11DeviceContext*    pContext)
{
	PreRender(pContext);
	Draw(pContext, this);
	return true;
...
bool		ABoneObj::Load(ID3D11Device* pDevice,	const TCHAR* szLoadName,		const TCHAR* pLoadShaderFile,		bool bThread)
{
	FILE	*fp;
	...
	for (int ibip = 0; ibip < m_Scene.iNumMesh; ibip++)
	{
		...
		fread(&iCount2, sizeof(int), 1, fp);
		fread(szParentName, sizeof(TCHAR) * iCount2, 1, fp);
	...
	if (!Set(pDevice, pLoadShaderFile,0,true))
	{
		return false;
	}
	return true;
...
bool ABoneObj::UpdateBuffer()
{
	...
	size_t ibOffset = 0;
	{
		for (DWORD dwObject = 0; dwObject < m_pMesh.size(); dwObject++)
		{
			...
			g_pImmediateContext->UpdateSubresource(m_dxObj.g_pVertexBuffer.Get(), 0, &pMesh->m_dxObj.m_BoxVB, (void*)&pData->m_VertexArray.at(0), 0, 0);
