...
bool ABoxObj::CompileShader(ID3D11Device* pDevice, const TCHAR* pShaderFile)
{
	HRESULT hr = S_OK;
	AModel::CompileShader(pDevice, pShaderFile);
...
bool ABoxObj::SetSO(ID3D11Device* pd3dDevice, ID3D11DeviceContext*  pContext)
{
	DX::ApplyDSS(pContext, DX::ADxState::g_pDSSDepthDisable);
...
bool ABoxObj::RenderMesh(ID3D11DeviceContext*  pContext, bool bFirstRender)
{
	PreRender(pContext);
...
bool ABoxObj::Render(ID3D11DeviceContext*  pContext)
{
	PreRender(pContext);
